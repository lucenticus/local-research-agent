...
# Ещё одна отдельная коллекция — эфемерный пул дайджеста (agent/digest.py).
# Полностью пересобирается на каждый запрос с query (`rebuild`), т.к. это
# не накопительный индекс, а разовый рабочий набор "статьи за N дней":
# смешивать его с research/ask-индексами нельзя по той же причине, что
# описана выше для QDRANT_RESEARCH_COLLECTION.
QDRANT_DIGEST_COLLECTION = "digest_pool"
# Отдельная коллекция под eval-прогоны (evals/run_quality.py). Нужна не для
# изоляции от продовых данных, а для КОРРЕКТНОСТИ ЗАМЕРА: research-коллекция
# накопительная, и повторный прогон того же вопроса переиспользует всё, что
# скачал предыдущий — второй прогон выходит быстрее и «качественнее» просто
# потому, что он второй. Сравнение baseline↔after на такой коллекции
# бессмысленно, поэтому харнесс чистит свою коллекцию перед каждым вопросом.
QDRANT_EVAL_COLLECTION = "eval_research"
...
# Явного фолбэка на CPU под давлением памяти нет: эмбеддинг корпуса и
# резидентная LLM надолго одновременно не грузятся.
EMBED_DEVICE = "mps"
...
# Глубокий разбор статьи по её PDF (§ пользовательский запрос: основные
# результаты, сравнения с аналогами, ссылки на код/HuggingFace) — качается
# полный текст, режется на секции и bounded-LLM'ом сводится в разбор.
DIGEST_PDF_ANALYSIS = True
# Жёсткий потолок контекста для этого разбора. Полный текст статьи — это
# десятки тысяч символов, а раздутый контекст надувает KV-кэш (§1 CLAUDE.md,
# главный риск OOM на 16ГБ). Совпадает по смыслу с MAX_SYNTHESIS_CONTEXT_CHARS,
# но задан отдельно: это другой сценарий с другим объёмом входа.
DIGEST_PDF_CONTEXT_CHARS = 6000
# Ответ длиннее обычного саммари — тут три раздела (результаты, сравнения,
# ограничения), а не один абзац. 1000, а не 700: на реальной статье с
# таблицей результатов разбор обрывался на полуслове посреди перечисления
# метрик, ровно на границе в 700.
DIGEST_PDF_MAX_TOKENS = 1000
# Когда в дайджесте задан query: ArxivSource.recent() больше не ANDит
# keyword в сам arXiv-запрос (на некоторых формулировках это ловило таймаут
# — составной булев запрос из keyword'ов + 6 категорий в OR оказывался
# слишком дорогим на стороне arXiv). Вместо этого тянем весь пул статей за
# те же `days` только по категориям (дешёвый, предсказуемый по времени
# запрос) и ранжируем его локально — см. agent/digest.py. Отдельного лимита
# на размер пула нет: окно `days` само его ограничивает (выдача arXiv
# отсортирована по дате, пагинация останавливается на границе окна), а сам
# пул кэшируется в Qdrant между прогонами.
# Реранкать весь пул из тысяч статей нельзя — реранкер идёт последовательно,
# по forward-проходу на статью (~0.15с на реальных абстрактах = ~2.5 мин).
# Поэтому пул сначала прогоняется через тот же гибридный dense+sparse поиск,
# что и research/ask (эмбеддинг батчами заметно дешевле реранка на статью),
# и реранкер получает уже только эти DIGEST_QUERY_HYBRID_K кандидатов.
DIGEST_QUERY_HYBRID_K = 20
# Сколько самых релевантных запросу статей показывать после реранка top-20 —
# по запросу пользователя, топ-5.
DIGEST_QUERY_TOP_K = 5
...
# --- Сборка контекста синтеза (agent/research_runner.py) ---

# Собирать контекст по каждому подвопросу отдельно, а не одним retrieval по
# вопросу целиком. Замер показал, что потолок покрытия подтем стоит именно
# здесь: TOP_K_RETRIEVE чанков, отобранных по полной строке запроса,
# группируются вокруг одной-двух граней, и остальные до синтеза не доезжают
# независимо от того, что нашла воронка.
RETRIEVE_PER_SUBQUESTION = True
# Общий потолок чанков после слияния по граням. Дальше всё равно действует
# MAX_SYNTHESIS_CONTEXT_CHARS — этот лимит про то, чтобы одна грань не
# съела всю выдачу, а не про размер контекста.
SYNTHESIS_MAX_CHUNKS = 8

# --- Роутер (agent/router.py), узел 2 ---

# Меньше этого числа содержательных слов (после отбрасывания служебных) —
# запрос не ищется, нужно уточнение.
ROUTER_MIN_CONTENT_WORDS = 2
# Пробовать ли локальный индекс, прежде чем идти в интернет.
ROUTER_PROBE_LOCAL_INDEX = True
# Сколько кандидатов брать из локального индекса на пробу.
ROUTER_PROBE_K = 5
# Насколько уверенно локальный корпус должен отвечать, чтобы обойтись `ask`.
# Выше общего FUNNEL_MIN_RERANK_SCORE (0.5) намеренно: ошибка в сторону `ask`
# отдаёт плохой ответ молча, ошибка в сторону `research` — всего лишь дольше.
ROUTER_ASK_MIN_SCORE = 0.8

# --- Планировщик (agent/planner.py) ---

# Bounded LLM-разбор вопроса на аспекты. ВЫКЛЮЧЕН по результату замера.
#
# Гипотеза была: многогранный вопрос остаётся одним подвопросом, цикл
# закрывается на первой найденной грани, отсюда низкое покрытие подтем.
# Первый замер её подтверждал (0.42), но он делался на НАКОПИТЕЛЬНОЙ
# research-коллекции. После изоляции индекса (QDRANT_EVAL_COLLECTION)
# оказалось, что без декомпозиции покрытие 0.75, а с ней — 0.584.
#
# ВАЖНО про то, как это мерилось. Первый вывод («декомпозиция ухудшает
# покрытие на 0.166») оказался ШУМОМ. Прогон той же самой конфигурации
# дважды, при temperature=0 и чистом индексе, на 4 многогранных вопросах:
#   A1: покрытие подтем 0.333 · citation 0.720 · медиана 115с
#   A2: покрытие подтем 0.167 · citation 0.624 · медиана 150с   <- та же конфигурация!
# Разброс между идентичными прогонами — 0.166, ровно та величина, которую я
# принял за эффект. Источник разброса не LLM (температура нулевая), а живые
# источники: каждый прогон находит другие статьи.
#
# С планировщиком + пофасетной сборкой: покрытие 0.222 — внутри разброса,
# то есть эффекта НЕ ВИДНО ни в плюс, ни в минус. Единственное, что похоже
# на сигнал: citation coverage выше базовой в обоих замерах (0.846 против
# 0.624-0.720), но и это ниже порога уверенности.
#
# Вывод: при разрешении текущего харнесса (~0.17 по покрытию на 4 вопросах)
# решить про декомпозицию нельзя. Нужны фикстуры источников, иначе их
# разброс перекрывает любой эффект. До тех пор — выключено: включать
# непроверенное изменение, которое стоит +LLM-вызов на запрос, незачем.
# Код и тесты оставлены: это заготовка с записанным замером, а не мёртвая
# ветка.
PLANNER_LLM_DECOMPOSE = False
# Потолок на число подвопросов. Каждый лишний — это полный проход discovery по
# всем источникам плюс триаж, то есть прямая трата бюджета
# (DEFAULT_BUDGET_MAX_SECONDS ниже), поэтому потолок низкий.
PLANNER_MAX_SUBQUESTIONS = 4
# Ниже этого числа слов вопрос на аспекты не разбирается. Дело не в экономии:
# на коротком бессмысленном входе модель не говорит "делить нечего", а
# сочиняет правдоподобные подвопросы про что угодно — поймано тестом цикла,
# где "question?" превратился в три вопроса про цепочки поставок.
PLANNER_MIN_QUESTION_WORDS = 5
# Границы «похоже на подвопрос»: короче — не ищется, длиннее — модель ушла в
# рассуждения вместо списка.
PLANNER_MIN_FACET_WORDS = 3
PLANNER_MAX_FACET_WORDS = 25
...
# --- Здоровье источников (agent/state.py::SourceHealth, узел 8) ---

# Сколько неудач ПОДРЯД должен дать источник, чтобы его перестали звать до
# конца прогона. Два, а не один: разовый таймаут случается у здорового
# источника, и вычёркивать его из-за одной осечки — терять данные там, где
# надо было просто повторить. Но и терпеть бесконечно нельзя: неудачная
# попытка Semantic Scholar стоит ~6 секунд одного лишь сна между ретраями,
# и на 4 подвопросах × 3 прохода это больше минуты ожидания источника,
# который уже дал понять, что недоступен.
