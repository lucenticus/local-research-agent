...
`gaps` (agent/loop.py) — подвопросы, не закрытые до исчерпания бюджета:
честно передаём их модели, чтобы ответ отражал реальные пробелы, а не делал
вид, что покрыто всё (цикл обязан завершаться по budget, даже если остались
пробелы — см. docstring `agent/state.py`).
"""
