...
Использует те же эталонные пары, что и eval_retrieval.py. Recall@3 там уже
насыщен (8/8 и для dense, и для hybrid — корпус мал), поэтому здесь смотрим на
более строгую метрику: попадает ли ожидаемый источник на позицию #1. Именно
это реранкер и должен улучшать поверх грубого RRF-слияния.
...
def main() -> None:
    ...
    for question, expected in EVAL_PAIRS:
        ...
        candidates = store.search_hybrid(question, query_vector, k=config.RERANK_CANDIDATES_K)
        reranked = rerank.rerank(question, candidates, top_n=config.TOP_K_RETRIEVE)

        hybrid_ok = hit_at_1(candidates, expected)
        reranked_ok = hit_at_1(reranked, expected)
