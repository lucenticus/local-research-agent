...
даёт 0.333 против 0.0 — он один и создаёт весь разброс агрегата 0.083. Все
внешние входы заморожены, температура нулевая, id чанков детерминированы
(`qdrant_store.chunk_id_for`), промахов ноль — остаток лежит ниже уровня,
