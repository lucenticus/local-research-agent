...
def main() -> None:
    ...
    if mode == MODE_RECORD:
        ...
        if failures:
            # Замер по такой фикстуре валиден, но меряет систему БЕЗ этого
            # источника — и узнать об этом надо сейчас, а не после того, как
            # выводы уже сделаны.
            print("\n⚠ источники, падавшие при записи (их падение заморожено):")
            for name, n in sorted(failures.items()):
                print(f"    {name:<18} {n} из {len(cases)} вопросов")
        if not len(cache):
